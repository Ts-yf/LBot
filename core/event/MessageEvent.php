...
class MessageEvent
{
    ...
    private function makeMsg($data)
    {
        ...
        $stream_outer_index = 0; // 用于管理跨多个 stream payload 的 index

        foreach ($normalizedData as $segmentInput) {
            $payload = [
            ...
            $currentSegment = $segmentInput;

            if (!is_array($currentSegment) || !isset($currentSegment['type'])) {
                error_log("MessageEvent::makeMsg: Invalid segment structure: " . json_encode($currentSegment));
                continue; // 跳过这个无法处理的消息段
            }
            ...
            switch ($currentSegment['type']) {
                case 'text':
                    ...
                    if (preg_match_all('/https?:\/\/[\w\-]+(\.[\w\-]+)+[\w\-\.,@?^=%&:\/~\+#!]*[\w\-\@?^=%&\/~\+#]/', $payload['content'], $matches)) {
                        if (!empty($matches[0])) { // $matches[0] 包含所有完整匹配
                ...
                case 'reply':
                    if (isset($currentSegment['id'])) {
                        if (strpos($currentSegment['id'], 'event_') === 0) {
                            ...
                            if(isset($payload['msg_id'])) unset($payload['msg_id']);
                        } else {
    ...
    private function getmedia($data, $type)
    {
        ...
        if (isset($responseData['file_info'])) {
            ...
            error_log("MessageEvent::getmedia: Failed to upload media or missing file_info. Response: " . $result);
    ...
    private static function upload_to_bilibili($imageData)
    {
        ...
        if (empty($csrfToken) || empty($sessData)) {
...
function getFileBase64($file)
{
    ...
    // 如果已经是 Base64 字符串
    if (str_starts_with($file, 'base64://')) {
